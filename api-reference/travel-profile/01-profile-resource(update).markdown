...
## <a name="a1">Update a travel profile</a>

This endpoint allows for updating travel profile information for the specified user. The travel profile includes information such as the user's identity, contact information, rate preferences and discount codes, preferences by travel type, and custom fields.

###  URI

<code>https://{InstanceURL}/api/travelprofile/v2.0/profile</code> **<- need to update this

###  Request parameters

Request parameters to identify the user are required. To identify a specific user, use  Login ID or XML Sync ID, you can specify the following request parameters:
...
| Element Name | Data Type | Description |  Scope | Min Length | Max  Length | Accepts Unicode | Comment |
| ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- |
|  `NamePrefix` |  `string` |  The user's name prefix. Format: nvarchar | | | 60 | yes | |
|  `FirstName` |  `string` |  The user's first name. Format: nvarchar | | | 60 | yes | |
|  `MiddleName` |  `string` |  The user's middle name. Format: nvarchar | | | 60 | yes | |
|  `LastName` |  `string` |  The user's last name. Format: nvarchar | | | 60 | yes | |
|  `NameSuffix` |  `string` |  The user's name suffix. Format: nvarchar | | | 60 | yes | |
|  `PreferredName` |  `string` |  The user's preferred name. Format: nvarchar | | | 60 | yes | |
|  `JobTitle` |  `string` |  The user's job title. Format: nvarchar | | | 255 | yes | |
|  `CostCenter` | `string` | The user’s cost center. Format: nvarchar | Company Details | | 25 | yes | |
|  `CompanyEmployeeID` | `string` | The user’s employee ID. Format: nvarchar | Company Details | | 48 | yes | Value must be unique within company.
|  `Division` | `string` | The user's division. Format: nvarchar | Company Details | | 60 | yes | Value must already be setup within a company.
|  `PreferredLanguage` |  `string` |  The user's preferred language locale. Example: United States English is en-US. Format: varchar | | | 20 | yes | |
...
|  `CountryCode` |  `string` |  The country code in from the[ ISO 3166-1 alpha-2 country code][8] specification. Format: char | | | 2 | yes | ?? need to check on this |
|  `CompanyName` |  `string` |  The user's company name. Format: nvarchar | | |  | | Not available to update. |
|  `CompanyID` | `string` | The user's company ID. Format: varchar | | |  |  | Not available to update. |
|  `RuleClass` |  `string` |  The user's rule class. Format: nvarchar | | | | yes | Value must already exist in the company. |
|  `MedicalAlerts` | `string` |  The user's medical alerts. Format: nvarchar | Medical Alerts | | 255 | yes | 
|  `GDSProfileName` |  `string` |  The user's GDS profile name. Format: varchar(60) | TMC Specific | | 60 | yes | |

* **NOTE**:  if no middle name is provided, the element NoMiddleName tag in the TSAInfo section will be set to false.
...
|  Element Name |  Data Type |  Description | Min Length | Max  Length | Accepts Unicode | Comment |
| ----- | ----- | ----- | ----- | ----- | ----- | ----- |
| `Name`  |  `string`  | The emergency contact name> Format: nvarchar | | 255 | yes |  |
| `RelationshipUpdateMode`  |  `enum`  | Whether to delete the relationship or not. | | | |  |
| `Relationship` |  `enum`  | The relationship to the user. Values are: Spouse, Brother, Parent, Sister, LifePartner, Other | | | | |
| `Phone`  | `list` | The emergency contact’s phone number in list format. This contains a required “type” attribute with values of DayTime and Alternate. | |  |  |  |
| `Phone - CountryCode`  | `String` | The emergency contact’s country code. Format: string | | | No |  |
| `Phone - AreaCityCode`  | `String` | The emergency contact’s area code. Format: string | | | No |  |
| `Phone - PhoneNumber`  | `String` | The emergency contact’s phone number. | | 60 | No |  |
| `Phone - Extension`  | `String` | The emergency contact’s extension. Format: string | | | No |  |
| `Phone - Type`  | `enum` | The emergency contact’s phone number. This contains a “type” attribute with values of DayTime and Alternate. Format: nvarchar | | 60 | yes |  |

* need more info on how address is supposed to be passed in...

| `Street` |  `string` | Street Address. Format: nvarchar(max) | | | No | fix me |
| `City` |   `string` | The city name. Format: nvarchar(30) | | | No | fix me  |
| `StateProvince` |   `string` | The state or province. Format: nvarchar(30) | | | No | fix me |
| `CountryCode` | `string` | The country code in from the ISO 3166-1 alpha-2 country code specification. Format: char(2) | | | No | fix me |
| `PostalCode` | `string` | The postal code. Format: nvarchar(20) | | | No | fix me |

* **NOTE**:  RelationshipUpdateMode is only used for Delete. Store and StoreAndDelete values are ignored.
* **NOTE**:  If multiple values of the same telephone type are provided, only the last one will be saved.
* **NOTE**:  CountryCode and AreaCityCode cannot be updated
***
...
|  Element Name |  Data Type |  Description | Min Length | Max  Length | Accepts Unicode | Comment |
| :----- | :----- | :----- | :----- | :----- | :----- | :----- |
|  `TelephonesUpdateMode` | `enum` | Type of update mode. Values are: Store, StoreAndDelete, and Delete | | | No |  |
|  `Telephones` | `list` | | | | No |  |
|  `Telephone - Type attribute` | `enum?` | Type of phone. Values are: Home, Work, Fax, Pager, Other, and Cell | | | No | verify me |
|  `Telephone - ContactOptIn attribute` |  `boolean` |  Whether the user has opted in to being contacted on this phone. Only appears when the phone type is Cell. Format: **true**\|**false** | | | No | |
|  `Telephone - PrimaryMobile attribute` | `boolean` | This is the user's preferred mobile device. Format: **true**\|**false** | | | No | |
|  `Telephone - CountryCode` |  `string` |  The country code in from the[ ISO 3166-1 alpha-2 country code][8] specification. Format: Char | | 2 | No | |
|  `Telephone - PhoneNumber` |  `string` | The phone number as entered by the user, which may contain characters such as () or -. Format: nvarchar <br> **NOTE**: The user phone number may sometimes be incorrectly parsed if there are data input issues.| | 60 | No |  |
|  `Telephone - Extension` |  `string` |  The phone extension. Format: nvarchar | | 60 | No |  |
|  `Telephone - MobileDevice` | `string` |  The OS of the mobile device. Format: varchar | | 25 | No |  |
|  `Telephone - MobileName` | `string` | The name the user assigned to the mobile device. Format: nvarchar | | 255 | No |  |

* **NOTE**
  * Type:
    * TravelAdmin, Supervisor, and SupervisorFax cannot be changed
  * Cell
    * DeviceName, DeviceType, ContactOptIn can be updated
    * Cell phones are identified by PhoneNumber and CountryCode
  * All Other Types (Other, Pager, Fax, Home, Work, Work2)
  ...
  * TelephonesUpdateMode:
    * StoreAndDelete – Will delete any cell phones that are not included in the list, but will not remove any of the other types of phone numbers
    *Delete – If there are no entries in Telephones, this will delete all phone numbers from the user, otherwise will remove only the specific cell phone numbers, or specified non-cell types
    *Store – Cell Phones will be added/updated, others will just be updated

  * IsPrimaryMobile:
    * If there is not exactly 1 IsPrimaryMobile specified in changing cell phones, then all cell phone changes will be rejected but other phones will still be updated.
